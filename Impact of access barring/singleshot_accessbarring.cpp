...
int main(){
	int arr[] = {60,120,180,240,300,360,420,480,540,600,660,720,780,840,900,960,1020,1080,1140,1200};
	...
	int P[3] = {12,12,12};
	ofstream myfile;
	myfile.open ("singleshot_accessbaring0.txt");
    	...
    	while(ack[0]+ack[1]+ack[2]+drop[0]+drop[1]+drop[2] < N){
			...
			for(int j=0;j<N;j++){
				...
				if(flag[j] == 0 && UE_CElevel[j] == 0 && (slot-T_start)%T_b[0] == 0 && packet_arrival[j] < slot && param < alpha1){
					...
					packet_arrival[j] += (0.7+0.5*distr7(generator7)/100)*4000;
				}
			...
			for(int j=0;j<P_total;j++){
				if(flagSlot[j] == 1){
					...
					meandelay[initial_CElevel[UE]] += ((double)packet_arrival[UE]-T_start)/1000; 
				}
