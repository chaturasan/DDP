...
//	int arr[] = {24300,24330,24360,24390,24420,24450,24480,24510,24540,24570,24600};
	...
	double lam[] = {0.0000033};
	int iterations = 1;
	int size1 = sizeof(lam)/sizeof(lam[0]);
	ofstream myfile;
	myfile.open ("bernoulli1.txt");
    ...
    for(int a=0;a<size1;a++){
    	...
    	/*std::random_device rd; 
  		...
  		cout << (long long)rng (rnd_gen) << endl;*/
    	...
    	while(a1-- > 0){
			...
			int s;
			cout << distribution(generator6)*1000 << endl;
			for(int i=0;i<N;i++){
				...
				//	packet_arrival[i] = (long long)rng (rnd_gen);
					packet_arrival[i] = distribution(generator6)*T_b[0];
					...
					//packet_arrival[i] = (long long)rng (rnd_gen);
					packet_arrival[i] = distribution1(generator7)*T_b[1];
					...
					//packet_arrival[i] = (long long)rng (rnd_gen);
					packet_arrival[i] = distribution2(generator8)*T_b[2];
    		...
    		while(slot < 43200008){
				...
				for(int j=0;j<P_total;j++){
					if(flagSlot[j] == 1){
						...
						if(initial_CElevel[UE] == 0) packet_arrival[UE] = packet_arrival[UE]+distribution(generator6)*T_b[0];//packet_arrival[UE] = packet_arrival[UE] + (long long)rng (rnd_gen);
						else if(initial_CElevel[UE] == 1) packet_arrival[UE] = packet_arrival[UE]+distribution1(generator7)*T_b[1];//packet_arrival[UE] + (long long)rng (rnd_gen);
						else if(initial_CElevel[UE] == 2) packet_arrival[UE] = packet_arrival[UE]+distribution2(generator8)*T_b[2];//packet_arrival[UE] + (long long)rng (rnd_gen);
						...
						for(int k=0;k<slotUE[j].size();k++){
							...
							if(attempts[UE] == 0 && CElevel < 2){
								...
								if(attempts[UE] == 0){
									...
									else if(initial_CElevel[UE] == 2) packet_arrival[UE] = packet_arrival[UE]+distribution2(generator8)*T_b[2];//packet_arrival
									initial_arrival[UE] = packet_arrival[UE];
